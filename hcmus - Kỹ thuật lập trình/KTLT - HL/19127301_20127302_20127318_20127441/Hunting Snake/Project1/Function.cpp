...
void drawbox(int x, int y, int width, int height)
{
	...
	GotoXY(x - 4, y); cout << char(218);
	GotoXY(x - 4, y + height); cout << char(192);
	GotoXY(x + width - 4, y); cout << char(191);
	GotoXY(x + width - 4, y + height); cout << char(217);
...
void StartGame()
{
	...
	STATE = 1;
	PlaySound(L".\\Music\\main.wav", NULL, SND_FILENAME | SND_LOOP | SND_ASYNC);
}
...
{
	STATE = 0;
	PlaySound(L".\\Music\\dead.wav", NULL, SND_ASYNC);
	BlinkSnake();
}
...
void GenerateFood()
{
	...
	for (int i = 0; i < MAX_SIZE_FOOD; i++)
	{
		do
		{
			x = rand() % (WIDTH_CONSOLE - 25) + 4;
			y = rand() % (HEIGH_CONSOLE - 7) + 4;
